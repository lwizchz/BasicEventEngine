...
*/
bool BEE::is_mouse_inside(const Instance& instance) const {
...
/*
* BEE::keystrings_populate() - Populate the map of keycodes
*/
int BEE::keystrings_populate() {
	if (keystrings_keys.empty()) {
		keystrings_keys = std::map<std::string,SDL_Keycode>{
	...
	if (keystrings_strings.empty()) {
		keystrings_strings = std::map<SDL_Keycode,std::string>{
...
#undef stringkey
/*
* BEE::keystrings_get_key() - Return the SDL keycode from the given string
*/
SDL_Keycode BEE::keystrings_get_key(const std::string& key) {
	if (keystrings_keys.empty()) {
		keystrings_populate();
	}

	if (keystrings_keys.find(key) != keystrings_keys.end()) {
		return keystrings_keys[key];
	}

	return SDLK_UNKNOWN;
}
/*
* BEE::keystrings_get_string() - Return the SDL keycode from the given string
*/
std::string BEE::keystrings_get_string(SDL_Keycode key) {
	if (keystrings_strings.empty()) {
		keystrings_populate();
	}

	if (keystrings_strings.find(key) != keystrings_strings.end()) {
		return keystrings_strings[key];
	}
